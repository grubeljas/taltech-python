...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for k, v in flipped.items():
        v = v.sort()
    sorted_hashtags = sorted(flipped.items(), key=lambda x: x[1])
...
if __name__ == '__main__':
    tweet1 = Tweet("@realDonaldTrump", "Despite the negative press covfefe #bigsmart", 1249, 3)
    tweet2 = Tweet("@elonmusk", "Technically, alcohol is a solution #bigsmart", 366.4, 233)
    tweet3 = Tweet("@CIA", "We can neither confirm nor deny that this is our first tweet. #heart", 2192, 238)
    tweet4 = Tweet("@CIA", "We can neither confirm nor deny that this is our first tweet. #bich", 2192, 238)
